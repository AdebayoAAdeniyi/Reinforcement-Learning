...
for name, get_phi in zip(
    ...
    if name == "Poly":
        phi_selected = phi[:, 3:4]  # Select the 4th column (feature 3, index 3)
    else:
    ...
    axs[0].plot(x, y, label='True Function', color='blue')
    axs[1].plot(x, y_pred, label=f'Approximation: {name}', color='orange')
